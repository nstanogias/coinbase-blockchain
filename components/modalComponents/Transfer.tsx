...
const Transfer = ({ setAction, twTokens, selectedToken, walletAddress }) => {
  ...
  const [activeTwToken, setActiveTwToken] = useState<Token>();
  const [imageUrl, setImageUrl] = useState('');
  const [balance, setBalance] = useState('Fetching...');

  useEffect(() => {
    twTokens.map((token) => {
      if (token.getAddress() === selectedToken.contractAddress) {
  ...
  const sendCrypto = async () => {
    if (activeTwToken && amount && recipient) {
      ...
      const result = await activeTwToken.transfer(recipient, amount.toString());
