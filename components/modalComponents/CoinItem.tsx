...
const CoinItem = ({ token, sender, setAction, selectedToken, setSelectedToken, twTokens }) => {
  ...
  useEffect(() => {
    const getBalance = async () => {
      ...
      twTokens.map((twToken) => {
        if (twToken.getAddress() === token.contractAddress) {
  ...
  return (
    ...
    >
      ...
      <IsSelected>{Boolean(selectedToken.contractAddress === token.contractAddress) && <FaCheck />}</IsSelected>
    </Wrapper>
  );
};
